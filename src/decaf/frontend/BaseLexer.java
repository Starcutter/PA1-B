...
public abstract class BaseLexer {
    ...
    protected int imgConst(String cval) {
		try {
			int value = Integer.decode(cval.substring(0, cval.length() - 1));
			setSemantic(getLocation(), SemValue.createLiteral(
					Tree.IMG, cval));
		} catch (NumberFormatException e) {
			Driver.getDriver().issueError(
					new IntTooLargeError(getLocation(), cval.substring(0, cval.length() - 1)));
		}
		return Parser.LITERAL;
	}
